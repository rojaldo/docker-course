...
.Los verbos comunes en las reglas de RBAC incluyen:
- `get`: Leer un recurso específico.
- `list`: Listar todos los recursos de un tipo.
...
- `deletecollection`: Eliminar múltiples recursos de un tipo.
...
Esto desplegará todo el stack de monitorización en el namespace monitoring. Puedes personalizar la instalación usando valores de Helm.

.Para comprobar el estado de instalación:
[source,bash]
----
kubectl get all -n monitoring
----

.Para acceder a la interfaz web de Prometheus:
[source,bash]
----
...
Grafana es una herramienta de visualización que se integra con Prometheus para crear dashboards y paneles de control. Para desplegar Grafana en Kubernetes, puedes usar el chart oficial de Helm.

.Para añadir el repositorio de Grafana:
[source,bash]
----
...
.Para obtener el password de admin:
[source,bash]
----
...
.Prometheus guarda datos en series temporales, cada una compuesta por:
- Un nombre de métrica, por ejemplo: `http_requests_total`, `node_cpu_seconds_total`, etc.
- Un conjunto de etiquetas (labels) en pares clave-valor, por ejemplo: `{method="GET", status="200"}`

.Una consulta simple retorna las series que coinciden con un filtro:
[source,promql]
----
http_requests_total{method="GET"}
----

Esta consulta devuelve todas las métricas `http_requests_total` cuyo `method` sea `GET`.

.PromQL trabaja con dos tipos principales de vectores:
- Vector instantáneo: devuelve el último valor de cada serie.
- Vector de rango: contiene los valores de una métrica dentro de un intervalo definido con corchetes `[]`.
...
Obtiene los valores de los últimos cinco minutos.


.PromQL incluye diversas funciones matemáticas y estadísticas, como:
- `rate(metric[interval])` → tasa de cambio por segundo.
- `increase(metric[interval])` → incremento total en ese periodo.
- `sum()`, `avg()`, `max()`, `min()` → funciones de agregación.
- `count()` → cuenta el número de series.
- `stddev()`, `stdvar()` → desviación estándar y varianza.
- `topk()`, `bottomk()` → los k valores más altos o bajos.
- `histogram_quantile()` → calcula cuantiles a partir de histogramas.
- `label_replace()` → modifica etiquetas usando expresiones regulares.
- `abs()`, `ceil()`, `floor()`, `round()` → funciones matemáticas básicas.
- `time()` → devuelve el timestamp actual en segundos.
...
Calcula la tasa de peticiones HTTP por segundo durante los últimos cinco minutos.

.Es posible filtrar resultados según etiquetas específicas:
[source,promql]
----
node_cpu_seconds_total{job="backend", mode!="idle"}
----

Filtra las métricas donde `job` sea backend y `mode` no sea idle.
...
Selecciona métricas con códigos de estado HTTP que comienzan por 4.

.PromQL dispone de operadores aritméticos y lógicos:
- Aritméticos: `+`, `-`, `*`, `/`
- Lógicos: `and`, `or`, `unless`
- Comparación: `==`, `!=`, `>`, `<`, `>=`, `<=`
- Agrupación: `by`, `without`
- `on`, `ignoring`

.Ejemplo:
[source,promql]
----
(cpu_usage_user + cpu_usage_system) / cpu_count
----

Calcula el uso total de CPU por núcleo.


.Las funciones de agregación permiten agrupar métricas por etiquetas:
[source,promql]
----
sum(rate(http_requests_total[5m])) by (job)
----

Suma la tasa de peticiones HTTP agrupadas por el valor de la etiqueta `job`.


.Un ejemplo común en paneles de Grafana:
[source,promql]
----
avg(rate(container_cpu_usage_seconds_total{namespace="monitoring"}[5m])) by (pod)
----

Muestra el promedio de uso de CPU por `pod` dentro del namespace `monitoring`.
...
.Los conceptos de Helm son:
* **Chart**: Un paquete que contiene todos los recursos necesarios para desplegar una aplicación en Kubernetes.
* **Release**: Una instancia desplegada de un chart en un clúster.
